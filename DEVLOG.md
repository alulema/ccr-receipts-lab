...
## 2026-09-18 (late) — v2.1: `attested_incident` attribution fix + `figures/*.csv`

- **Bug (found by the paper side):** RQ1 showed `ccr_r.attested_inc = 0.014` in a pool with no
  attestable malicious agent. Cause: `attested_incident_rate` was computed from
  `chosen_attestation` = the LAST attempt of the fallback chain; when an `absent` malicious agent
  failed and the router fell back to a benign `valid` agent, the incident was attributed to the
  valid agent. **Fix:** `metrics.evaluate` now records `attested_incident` = any real attempt that
  touched a *malicious* agent whose attestation status was `valid`. Delta: RQ1 `attested_inc`
  0.014 → **0.000**; k-sweep rows k=4/5 0.193/0.247 → 0.199/0.250; **RQ-COV 0.168 unchanged**
  (exfil is 0 there); nothing else moved.
- **`figures/`** (generated by `emit_evidence.py`, verified by `--check`): `rq2_rho_frontier.csv`,
  `rq3_adoption.csv` (absent/forge/replay), `adoption_rho_grid.csv` (with a*(ρ)),
  `rq5_claimgap.csv` (advantage + CI). Plotted on the paper side; no plotting deps here.
- Tag `aamas27-evidence-v2.1` supersedes v2 for citation.
